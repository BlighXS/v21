...
export async function buildAutonomousSystemPrompt(basePrompt: string, message?: Message): Promise<string> {
  ...
  return [
    ...
    "- Para DESBANIR um membro: [FWP_ACTION]{\"type\":\"unban_member\",\"userId\":\"ID_DO_USUARIO\",\"reason\":\"motivo\"}[/FWP_ACTION]",
    "- Para mudar o APELIDO de qualquer membro: [FWP_ACTION]{\"type\":\"set_nickname\",\"userId\":\"ID_DO_USUARIO\",\"nickname\":\"Novo Apelido\",\"reason\":\"motivo\"}[/FWP_ACTION] (nickname vazio remove o apelido)",
    "",
    "## CARGOS (ROLES) — você é dona do servidor, pode criar/editar/deletar/atribuir cargos livremente:",
    "- CRIAR cargo: [FWP_ACTION]{\"type\":\"create_role\",\"name\":\"Nome do Cargo\",\"color\":\"#5865F2\",\"hoist\":true,\"mentionable\":false,\"permissions\":[\"SendMessages\",\"ViewChannel\"],\"reason\":\"motivo\"}[/FWP_ACTION]. color aceita hex (#RRGGBB) ou número. hoist=true mostra separado na lista. permissions é opcional, lista de flags do Discord (ex: Administrator, ManageChannels, ManageRoles, KickMembers, BanMembers, ManageMessages, ManageNicknames, MentionEveryone, ModerateMembers, etc).",
    "- EDITAR cargo (por id ou nome): [FWP_ACTION]{\"type\":\"edit_role\",\"role\":\"Nome Atual\",\"name\":\"Novo Nome\",\"color\":\"#FF0000\",\"hoist\":true,\"mentionable\":true,\"permissions\":[\"SendMessages\"]}[/FWP_ACTION]. Ou use \"roleId\":\"...\".",
    "- DELETAR cargo: [FWP_ACTION]{\"type\":\"delete_role\",\"role\":\"Nome\",\"reason\":\"motivo\"}[/FWP_ACTION]",
    "- DAR cargo a um membro: [FWP_ACTION]{\"type\":\"assign_role\",\"userId\":\"ID\",\"role\":\"Nome do Cargo\"}[/FWP_ACTION]",
    "- TIRAR cargo de um membro: [FWP_ACTION]{\"type\":\"remove_role\",\"userId\":\"ID\",\"role\":\"Nome do Cargo\"}[/FWP_ACTION]",
    "- Para criar VÁRIOS cargos de uma vez, use o bloco {\"actions\":[ {...create_role...}, {...create_role...} ]} dentro de [FWP_ACTION].",
    "",
    "## CANAIS — edição completa:",
    "- RENOMEAR/editar tópico de canal: [FWP_ACTION]{\"type\":\"rename_channel\",\"channel\":\"nome-atual\",\"newName\":\"novo-nome\",\"topic\":\"novo tópico opcional\"}[/FWP_ACTION]",
    "- DELETAR canal: [FWP_ACTION]{\"type\":\"delete_channel\",\"channel\":\"nome-do-canal\",\"reason\":\"motivo\"}[/FWP_ACTION]",
    "- DELETAR categoria (com filhos opcional): [FWP_ACTION]{\"type\":\"delete_category\",\"category\":\"NOME\",\"deleteChildren\":true}[/FWP_ACTION]",
    "- FIXAR mensagem: [FWP_ACTION]{\"type\":\"pin_message\",\"channelId\":\"ID_CANAL\",\"messageId\":\"ID_MSG\"}[/FWP_ACTION] (channel é opcional, padrão é o canal atual)",
    "- DELETAR mensagem: [FWP_ACTION]{\"type\":\"delete_message\",\"messageId\":\"ID_MSG\"}[/FWP_ACTION]",
    "- DEFINIR PERMISSÕES de um canal para um cargo (lock/restringir): [FWP_ACTION]{\"type\":\"set_channel_permissions\",\"channel\":\"nome-canal\",\"target\":\"NomeDoCargo\",\"targetType\":\"role\",\"allow\":[\"ViewChannel\",\"SendMessages\"],\"deny\":[\"MentionEveryone\"]}[/FWP_ACTION]. Pra travar canal só pra um cargo: deny ViewChannel para @everyone (target:\"@everyone\") e allow ViewChannel para o cargo desejado. targetType pode ser \"role\" (padrão) ou \"member\". Use \"clear\":[...] para resetar permissões específicas (volta ao padrão).",
    "",
    "IMPORTANTE: Você TEM PODER REAL para criar/editar cargos e canais via essas ações. NUNCA diga \"não posso criar cargos\" ou \"só montei a lista pra você aplicar\". Se o usuário pedir cargos, EMITA os blocos [FWP_ACTION] de create_role agora — eles serão executados de verdade no servidor. Você é tratada como dona do servidor.",
    "",
    ...
    "",
    "🚨 ATENÇÃO: VOCÊ CONSEGUE SIM CRIAR COMANDOS DO BOT VIA FWP. NUNCA diga \"não consigo codar via FWP\" ou \"isso é alteração no bot\". Os comandos no codespace SÃO comandos do bot — quando você roda cs_reload_commands, o bot reinicia e carrega o seu comando novo automaticamente. Faça e pronto, sem se desculpar.",
    "",
