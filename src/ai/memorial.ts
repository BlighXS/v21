...
export async function buildAutonomousSystemPrompt(basePrompt: string, message?: Message): Promise<string> {
  ...
  return [
    ...
    sourceTree,
    "",
    "[ACESSO AO CÓDIGO FONTE — SISTEMA DE DUAS PASSADAS]",
    "Você tem acesso de leitura e escrita a todo o código fonte do projeto. O fluxo funciona assim:",
    "",
    "PASSO 1 — LER o arquivo antes de qualquer edição:",
    "  [FWP_ACTION]{\"type\":\"read_source_file\",\"path\":\"src/ai/memorial.ts\"}[/FWP_ACTION]",
    "  Você pode ler qualquer arquivo em src/, artifacts/, lib/, data/ — inclusive arquivos do dashboard web e da API.",
    ...
    "  REGRA CRÍTICA: Quando você emitir um read_source_file, comece sua resposta com [SILENT] para NÃO enviar nenhuma mensagem ao usuário nessa passada.",
    "  NUNCA diga 'já volto', 'dá um segundo', 'vou buscar o arquivo' ou qualquer variação.",
    "  Na passada seguinte (quando o arquivo já estiver disponível), aí sim responda normalmente com o que foi feito.",
    "  IMPORTANTE: Ao ler um arquivo, NUNCA use fromLine/toLine sem necessidade — omita esses campos para ler o COMPLETO.",
    "  Você pode solicitar até 10 leituras de arquivo de uma vez e o sistema fará múltiplas rodadas de follow-up.",
    "",
    "PASSO 2 — ESCREVER o arquivo com o conteúdo modificado (apenas o dono pode confirmar isso):",
    "  [FWP_ACTION]{\"type\":\"write_source_file\",\"path\":\"src/arquivo.ts\",\"content\":\"CONTEÚDO COMPLETO DO ARQUIVO AQUI\"}[/FWP_ACTION]",
    "  Você pode escrever qualquer arquivo em src/, artifacts/fawer-web/src/, artifacts/api-server/src/, lib/, data/.",
    "  ARQUIVOS BLOQUEADOS (nunca tente alterar): src/index.ts, src/utils/config.ts, src/utils/sysinfo.ts,",
    "    src/utils/logger.ts, src/utils/restart.ts, src/utils/net.ts, src/utils/permissions.ts,",
    "    qualquer package.json, tsconfig, .env, faw.env, .replit, artifact.toml, pnpm-lock.yaml, replit.md.",
    "  CRÍTICO: o campo 'content' deve conter o arquivo INTEIRO, não só as partes alteradas.",
    "  Não resuma, não coloque comentários como '// resto do código aqui'. ESCREVA TUDO.",
    "",
    "PASSO 3 — Após a CONFIRMAÇÃO do usuário, você pode reiniciar o bot:",
    "  [FWP_ACTION]{\"type\":\"restart_self\",\"reason\":\"aplicar alterações em X\"}[/FWP_ACTION]",
    "  IMPORTANTE: restart_self é adiado automaticamente se houver escrita pendente de confirmação.",
    "",
    "- Para LISTAR arquivos de um diretório: [FWP_ACTION]{\"type\":\"list_source_files\",\"dir\":\"src/ai\"}[/FWP_ACTION]",
    "  Exemplos de dirs válidos: src, src/ai, artifacts/fawer-web/src, artifacts/api-server/src, lib.",
    "  O resultado da listagem também será injetado automaticamente.",
    "",
